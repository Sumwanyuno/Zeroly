          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex flex-col md:flex-row justify-center items-end gap-4 md:gap-6 mb-16 h-auto md:h-72">
            ...
            {topThree[1] && (
              <motion.div 
                ...
                tabIndex={0}
                aria-label={`Second Place: ${topThree[1].name} with ${topThree[1].points} EcoCoins.`}
              >
                <div className="relative mb-4">
                  ...
                  <Medal className="absolute -bottom-3 -right-3 w-8 h-8 text-slate-400 drop-shadow-md" aria-hidden="true" />
                </div>
                <div className="bg-card/80 dark:bg-card/75 backdrop-blur-xl border border-slate-300/30 p-6 rounded-t-2xl w-full text-center h-32 md:h-40 flex flex-col justify-end pb-6">
                  ...
                  <div className="flex items-center justify-center text-slate-400 font-semibold mt-1">
                    <Coins className="w-4 h-4 mr-1" aria-hidden="true" /> {topThree[1].points} EcoCoins
                  </div>
                </div>
              </motion.div>
            )}
            ...
            <motion.div 
              ...
              tabIndex={0}
              aria-label={`First Place Winner: ${topThree[0].name} with ${topThree[0].points} EcoCoins.`}
            >
              <div className="relative mb-6">
                ...
                <Crown className="absolute -top-6 left-1/2 -translate-x-1/2 w-10 h-10 text-yellow-400 drop-shadow-lg" aria-hidden="true" />
              </div>
              <div className="bg-card/85 dark:bg-card/80 backdrop-blur-xl border border-yellow-400/50 p-6 rounded-t-2xl w-full text-center h-36 md:h-48 flex flex-col justify-end pb-8 shadow-[0_-10px_40px_rgba(250,204,21,0.1)]">
                ...
                <div className="flex items-center justify-center text-yellow-500 font-bold mt-1 text-lg">
                  <Coins className="w-5 h-5 mr-1" aria-hidden="true" /> {topThree[0].points} EcoCoins
                </div>
              </div>
            </motion.div>
            ...
            {topThree[2] && (
              <motion.div 
                ...
                tabIndex={0}
                aria-label={`Third Place: ${topThree[2].name} with ${topThree[2].points} EcoCoins.`}
              >
                <div className="relative mb-4">
                  ...
                  <Medal className="absolute -bottom-3 -right-3 w-8 h-8 text-amber-600 drop-shadow-md" aria-hidden="true" />
                </div>
                <div className="bg-card/80 dark:bg-card/75 backdrop-blur-xl border border-amber-600/30 p-6 rounded-t-2xl w-full text-center h-32 md:h-36 flex flex-col justify-end pb-6">
                  ...
                  <div className="flex items-center justify-center text-amber-600 font-semibold mt-1">
                    <Coins className="w-4 h-4 mr-1" aria-hidden="true" /> {topThree[2].points} EcoCoins
                  </div>
                </div>
              </motion.div>
            )}
            ...
            {restOfLeaders.map((user, index) => {
              ...
              return (
                <motion.div key={user.name} variants={itemVariants}>
                  <Card 
                    className={`bg-card/80 dark:bg-card/75 backdrop-blur-xl border-border/50 hover:bg-card/90 transition-all group ${userInfo?.name === user.name ? 'ring-2 ring-primary border-primary' : ''}`}
                    tabIndex={0}
                    aria-label={`Rank ${rank}: ${user.name}. ${user.itemCount} items uploaded. ${user.points} EcoCoins. Tier: ${tierInfo.name}.`}
                  >
                    <CardContent className="p-4 sm:p-6 flex items-center justify-between">
                      <div className="flex items-center gap-4 sm:gap-6">
                        <div className="w-8 font-bold text-muted-foreground text-lg" aria-label={`Rank ${rank}`}>#{rank}</div>
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <div className="flex items-center gap-2 font-bold text-lg">
                          {user.points} <Coins className="w-4 h-4 text-primary" aria-hidden="true" />
                        </div>
                        <div 
                          className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full ${tierInfo.bg} ${tierInfo.color}`}
                          aria-label={`Tier: ${tierInfo.name}`}
                        >
