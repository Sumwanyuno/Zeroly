...
const ItemCard = ({ item, userId, onDelete, onEdit }) => {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <Card className="overflow-hidden h-full flex flex-col group bg-card/80 dark:bg-card/75 backdrop-blur-xl border border-border/40 hover:border-primary/30 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 relative">
        {/* Floating Glassmorphic Category Badge */}
        <span className="absolute top-3 right-3 z-10 bg-background/80 backdrop-blur-md border border-border/40 text-foreground text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider shadow-sm select-none">
          {item.category}
        </span>
        ...
        {isOwner && (
          <div className="absolute top-3 left-3 z-10 flex gap-2">
            <button
              ...
              className="bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 hover:border-primary/30 backdrop-blur-md w-9 h-9 rounded-full flex items-center justify-center shadow-sm cursor-pointer transition-all duration-200 hover:scale-105 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
              title="Edit Listing"
            ...
            <button
              ...
              className="bg-destructive/10 hover:bg-destructive/20 text-destructive border border-destructive/20 hover:border-destructive/30 backdrop-blur-md w-9 h-9 rounded-full flex items-center justify-center shadow-sm cursor-pointer transition-all duration-200 hover:scale-105 focus-visible:ring-2 focus-visible:ring-destructive focus-visible:outline-none"
        ...
        <Link 
          to={`/item/${item._id}`} 
          className="overflow-hidden block aspect-[4/3] relative group-hover:after:opacity-100 after:absolute after:inset-0 after:bg-gradient-to-t after:from-background/25 after:to-transparent after:opacity-0 after:transition-opacity after:duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
          ...
          <div className="flex flex-col h-full justify-between">
            <div>
              ...
              <h3 className="text-lg font-extrabold text-foreground tracking-tight line-clamp-1 group-hover:text-primary transition-colors duration-200">
                <Link 
                  to={`/item/${item._id}`} 
                  className="block w-full focus-visible:outline-none focus-visible:underline"
            ...
            <div className="flex items-center justify-between mt-4 pt-3.5 border-t border-border/20">
              ...
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-bold text-xs shadow-sm">
                <Coins className="w-3.5 h-3.5 text-emerald-500" aria-hidden="true" />
        ...
        <CardContent className="p-5 pt-4 mt-auto border-t border-border/30 bg-secondary/10">
          <div className="flex items-center gap-2 text-muted-foreground/85 hover:text-foreground transition-colors duration-200">
            ...
            <span className="text-xs font-medium truncate select-none" aria-label={`Location: ${item.address || "Local Sharing"}`}>
